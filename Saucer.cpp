...
void Saucer::init() {
	...
	setVelocity(Hero::speed); // 1 space left every 4 frames
...
int Saucer::eventHandler(const df::Event* p_e) {
	...
	if (p_e->getType() == df::COLLISION_EVENT) {
		...
		Role::getInstance().getServer()->sendMessage(df::DELETE_OBJECT, this);
		//df::DELETE_OBJECT
...
void Saucer::moveToStart() {
	...
	setVelocity(Hero::speed);
}
void Saucer::speedUp()
{
	Hero::speed = df::Vector(Hero::speed.getX() * 1.05f, Hero::speed.getY());
}
